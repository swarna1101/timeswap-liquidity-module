...
describe(' Staking Contract', function () {
	...
	beforeEach(async () => {
		...
		stakingContract = await staking.deploy(timeswapTokenContract.address, liqTokenContract.address);
	...
	describe('Staking Tests', () => {
		it('Should mint Timeswap token', async () => {
			...
			expect(await timeswapTokenContract.balanceOf(stakingContract.address)).to.equal(ethers.utils.parseUnits('1000000000', 18));
		});

		it('Should stake any liqToken', async () => {
			await liqTokenContract.mint(addr1.address, 1, 5, 0x00);
			await liqTokenContract.connect(addr1).setApprovalForAll(stakingContract.address, true);
